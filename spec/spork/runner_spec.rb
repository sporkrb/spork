...
describe Spork::Runner do
  def use_test_server(klass = Spork::TestFramework::RSpec)
    ...
    allow(Spork::TestFramework).to receive(:factory).and_return(@test_framework)
  end
  ...
  it "outputs a list of supported servers, along with supported asterisk" do
    allow(Spork::Server).to receive(:supported_test_frameworks).and_return([Spork::TestFramework::RSpec, Spork::TestFramework::Cucumber])
    allow(Spork::TestFramework::RSpec).to receive(:available?).and_return(true)
    allow(Spork::TestFramework::Cucumber).to receive(:available?).and_return(false)
